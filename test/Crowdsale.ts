...
import { BustadToken, Crowdsale, IERC20 } from "../typechain";
...
describe("Crowdsale", () => {
  ...
  describe("Buy with stable coin", () => {
    before(async () => {
      ...
      const dai = <IERC20>(
        await ethers.getContractAt("IERC20", DAIaddress, mockUser)
      );

      await transferTotalBalance(horde, mockUser.address, dai.address);
      await resetTokenBalance(mockUser, bustadToken.address);
